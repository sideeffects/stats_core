...
class Migration(DataMigration):
    ...
    depends_on = (
        ("houdini_stats", 
         "0021_auto__add_field_uptime_idle_time"),
    )

    def forwards(self, orm):
        """ 
        Load data from houdini_stats.MachineConfig to 
        ...
        for event in orm['houdini_stats.Event'].objects.using("stats").all():
            new_event = orm.Event(title = event.title, date = event.date, 
                                  description = event.description,
                                  show = event.show)
            new_event.save(using="stats")
        
        # Load data for machines
        for machine in orm['houdini_stats.Machine'].objects.using("stats").all():
            new_machine = orm.Machine(hardware_id = machine.hardware_id)
            new_machine.save(using="stats")
            
        # Load data for machine config
        for machine_config in orm['houdini_stats.MachineConfig'].objects.using(
                                                                 "stats").all():
            machine_hardware_id = machine_config.machine.hardware_id
            
            saved_machine = orm.Machine.objects.using("stats").get(
                                              hardware_id = machine_hardware_id) 
            # Create new machine config 
            new_machine_config = orm.MachineConfig(
                    config_hash = machine_config.config_hash, 
                    ip_address = machine_config.ip_address, 
                    machine = saved_machine,                       
                    creation_date = machine_config.creation_date,
                    graphics_card = machine_config.graphics_card,
                    graphics_card_version = machine_config.graphics_card_version,
                    operating_system = machine_config.operating_system,             
                    system_memory = machine_config.system_memory,
                    system_resolution = machine_config.system_resolution, 
                    number_of_processors = machine_config.number_of_processors,
                    cpu_info = machine_config.cpu_info, 
                    raw_user_info = machine_config.raw_user_info
            ) 
            new_machine_config.save(using="stats")
                
        for log in orm['houdini_stats.LogId'].objects.using("stats").all():
            machine_config_hash = log.machine_config.config_hash
            
            saved_machine_config = orm.MachineConfig.objects.using("stats").get(
                                              config_hash = machine_config_hash) 
            # Create new log id 
            new_logid = orm.LogId(log_id = log.log_id,
                                  machine_config = saved_machine_config)  
            new_logid.save(using="stats")
    ...
    complete_apps = ['houdini_stats', 'stats_main']
    symmetrical = True
